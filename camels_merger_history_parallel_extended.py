#!/usr/bin/env python
"""
CAMELS IllustrisTNG L50n512 -- per-snapshot interpolated formation-time catalogs (extended).

Same as camels_merger_history_parallel.py, but records additional per-snapshot subhalo /
group properties along each main-progenitor branch:

    M500c_snap        Group_M_Crit500 -> physical Msun  (1e10 Msun/h * 1e10 / h)
    SubhaloVelDisp    1-D velocity dispersion  [km/s]   (no h conversion)
    SubhaloVmax       V_max of the rotation curve [km/s] (no h conversion)
    SubhaloSpin       |spin| magnitude  [proper (kpc)(km/s)]  (norm of 3-vector, h out, x a)

All quantities are stored in physical/proper units. Per the TNG data specs
(https://www.tng-project.org/data/docs/specifications/) the raw fields are 1e10 Msun/h
(masses), comoving (ckpc/h)(km/s) (SubhaloSpin), and km/s (Vmax, VelDisp). Masses are
divided by h; spin is divided by h and multiplied by a=1/(1+z) per snapshot to give proper
distance; the km/s velocities already carry no h.

Runs the merger-tree analysis for a range of boxes (default SB35_0 .. SB35_1023) in parallel
across N CPU cores. Each box reads its own SubLink tree_extended.hdf5, selects z=0 centrals
above a mass cut, computes cubic-interpolated formation times (10/25/50/90% of z=0 M200c),
records the per-snapshot quantities above along each main-progenitor branch, and writes a
single HDF5 file with the cosmological + astrophysical parameters in the header.

Usage
-----
    python camels_merger_history_parallel_extended.py                 # all 1024 boxes, 8 cores
    python camels_merger_history_parallel_extended.py --start 0 --end 1023 --nproc 8
    python camels_merger_history_parallel_extended.py --overwrite     # redo finished boxes
"""
...
OUT_DIR   = '/home/jovyan/home/shmr_merger/camels_mass_history_extended'

# fields walked along the main-progenitor branch (must exist in tree_extended.hdf5)
TREE_FIELDS = ['SnapNum', 'SubfindID', 'SubhaloGrNr', 'Group_M_Crit200',
               'Group_M_Crit500', 'SubhaloMassType', 'SubhaloMassInRadType',
               'SubhaloSFR', 'SubhaloVelDisp', 'SubhaloVmax', 'SubhaloSpin',
...
def process_box(box_idx, overwrite=False):
    ...
    try:
        ...
        base_cols = ['GroupNumber', 'GroupFirstSub', 'M200c_snap', 'M200c_z0',
                     'M500c_snap', 'Mstar', 'ISM_mass', 'CGM_mass', 'fcgm', 'SFR',
                     'SubhaloVelDisp', 'SubhaloVmax', 'SubhaloSpin', 'SplashbackFlag']
        ...
        for gfs in roots:
            ...
            m500c = tree['Group_M_Crit500'] * 1e10 / h
            mstar = tree['SubhaloMassType'][:, 4] * 1e10 / h
            gas   = tree['SubhaloMassType'][:, 0] * 1e10 / h
            ism   = tree['SubhaloMassInRadType'][:, 0] * 1e10 / h
            cgm   = gas - ism
            sfr   = tree['SubhaloSFR']
            veldisp = tree['SubhaloVelDisp']                 # km/s (already physical)
            vmax    = tree['SubhaloVmax']                    # km/s (already physical)
            # |spin| of the 3-vector; raw is comoving (ckpc/h)(km/s).
            # -> divide h out and multiply by a=1/(1+z) per node -> proper (kpc)(km/s)
            a_node  = 1.0 / (1.0 + redshift[tree['SnapNum'].astype(int)])
            spin    = np.linalg.norm(tree['SubhaloSpin'], axis=1) / h * a_node
            ...
            for i in range(len(tree['SnapNum'])):
                ...
                c['M500c_snap'].append(float(m500c[i]))
                ...
                c['SubhaloVelDisp'].append(float(veldisp[i]))
                c['SubhaloVmax'].append(float(vmax[i]))
                c['SubhaloSpin'].append(float(spin[i]))
...
def save_box(catalogs, filename, box, basePath, base_snap, redshift, age,
    ...
    col_docs = {
        ...
        'SubhaloVelDisp': '1-D velocity dispersion of all member particles/cells [km/s]',
        'SubhaloVmax':    'maximum of the spherically-averaged rotation curve [km/s]',
        'SubhaloSpin':    'magnitude of the subhalo spin vector [proper (kpc)(km/s); h divided out, x a=1/(1+z) per snapshot]',
    ...
    with h5py.File(tmp, 'w') as f:
        f.attrs['title']       = 'CAMELS L50n512 interpolated formation-time catalog (extended)'
        ...
        f.attrs['spin_unit']   = 'proper (kpc)(km/s) (h divided out, x a=1/(1+z) per snapshot)'
